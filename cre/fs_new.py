...
def load_wkt_csv(in_filename,geom_col="geom",sep=',',crs = {'init': 'epsg:4326'},to_crs=""):
    df = pd.read_csv(in_filename,sep=sep)
...
def load_latlon_csv(in_filename,lon_col="longitude",lat_col="latitude",sep=',',crs = {'init': 'epsg:4326'},to_crs=""):
    df = pd.read_csv(in_filename,sep=sep)
    geometry = [Point(xy) for xy in zip(df[lon_col], df[lat_col])]
...
#find an address column in the given csv file and geocode it with the local geocoder then the census geocoder
#if all rows are the same city and state, given the city and state as parameters
def geocode_csv(in_filename,market_name,score=1,input_street_col="",input_num_col="",geom_street_col="",geom_num_col="",verbose=1,join_other_cols=[],input_subloc_col="",geom_subloc_col=""):
    df = load_file(in_filename)
    parcel_gdf = load_file("../"+market_name+"/"+"parcelInfo.csv")
    if input_subloc_col !="" and geom_subloc_col != "":
        df[input_subloc_col] = df[input_subloc_col].apply(lambda x: str(x).upper())
        df[geom_subloc_col] = df[geom_subloc_col].apply(lambda x: str(x).upper())
    df = fast_geocode_local(df,parcel_gdf,input_street_col=input_street_col,input_num_col=input_num_col,geom_street_col=geom_street_col,geom_num_col=geom_num_col,join_other_cols=['upin'],geom_subloc_col=geom_subloc_col,input_subloc_col=input_subloc_col)    
        ...
        for i in range(0,len(temp)):
            if threshold > 0:
                ...
                if overlap > threshold:
                    return dict(temp.iloc[i][output_attrs])
            else:
                return dict(temp.iloc[i][output_attrs])
    result = {}
...
def geocode_local(num,street,compl_geom_df,split_geom_df,input_split_val,geom_subloc_col,verbose=1,total=0):
    ...
    if dt.is_null(num):
        num = 0
    if geom_subloc_col != "" and input_split_val != "" and input_split_val in split_geom_df:
...
#def geocode_local(address,gdf,verbose=1,cache_val=0,cache_col="",cache={},total=0,join_other_cols=[]):
#    global gcount
#    if total > 0:
#       gcounts += 1
#       if gcounts % 10 == 0:
...
#        if len(join_other_cols) == 0:
#            return row['geometry'].centroid
#        else:
#            final_result = [row['geometry'].centroid]
#            for col in join_other_cols:
...
def fast_geocode_local(input_gdf,geom_gdf,input_street_col="",input_num_col="",geom_street_col="",geom_num_col="",verbose=1,total=0,join_other_cols=[],input_subloc_col="",geom_subloc_col=""):
    reset_counts()
    if input_num_col == "":
    ...
    join_cols = ['geometry']
    join_cols.extend(join_other_cols) 
    ...
    if input_subloc_col != "":
        print('splits...')
        input_gdf[input_subloc_col] = input_gdf[input_subloc_col].apply(lambda x: str(x).replace(".0",""))
        input_gdf['temp_addr'] = input_gdf['temp_addr'] + " " + input_gdf[input_subloc_col]
    if geom_subloc_col != "":
        geom_gdf[geom_subloc_col] = geom_gdf[geom_subloc_col].apply(lambda x: str(x).replace(".0",""))
        geom_gdf['temp_addr'] = geom_gdf['temp_addr'] + " " + geom_gdf[geom_subloc_col]
    ...
    split_df = ""
    if input_subloc_col != "":
        split_df = dt.split_df(geom_gdf,geom_subloc_col)
        print(result.columns)
        
    result['geometry'] = result.apply(lambda row: geocode_local(row[input_num_col],row[input_street_col],geom_gdf,split_df,dt.value_or_default(row,input_subloc_col),geom_subloc_col,total=n,verbose=verbose) if dt.is_null(row['geometry']) else row['geometry'],axis=1)
